...
async def on_ready():
    print("Ready!")
    await client.change_presence(status=discord.Status.online, activity=discord.Game('!sri help'))
    ...
    if "!sri" in message.content:
        if "!sri help" in message.content:
            ...
            onegai_list=[
                ...
                "下北沢鉄道という架空の鉄道の停車中放送を、次の形式に基づいて一つだけ出力してください。形式：「本日も、（列車運営会社）をご利用くださいましてありがとうございます。（番線番号）番線に停車中の電車は、（発車時）時（発車分）分発、（直通先路線）直通、（列車種別）、（駅名）行きです。この電車は（列車両数）両です。発車までしばらくお待ちください。」",
            ...
            prompt = onegai + chokutsu + "回答には一切のかぎかっこ記号を含めないでください。番線番号と特急の号数、電車の両数の数字は114,514,810からそれぞれランダムに選んでください。特急の放送を行う際には、特急名を必ず放送してください。" + "以下が下北沢鉄道の情報です。所有する路線：下北沢本線。発着番線：810番線,514番線,114番線。発車時刻：１９時１９分。停車する駅：備中田所神代、野獣邸、下北沢、東北沢、軍畑、田所、遠野、三浦、木村、谷岡。両数：114両,810両,514両。直通先：「中オオン!・総武線」、「ゲイ備線」、「迫真線」。列車種別：普通、急行、特急。特急の名称：田所あずさ、快速ホームライナー屋上。特急の号数：514,114,810号のいずれか。"
